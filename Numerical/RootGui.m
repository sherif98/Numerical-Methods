...
% Last Modified by GUIDE v2.5 07-May-2016 15:06:44
...
switch get(get(handles.method_selector,'SelectedObject'),'Tag')
    ...
    case 'false',
        ...
        set(handles.out_msg, 'String', message);
        if(strcmp(message, 'Success'))
            ...
            set(handles.output, 'Data', a);
        end
        
    case 'fixed',
        equ2 = strcat(equ, '+x');
        f2 = str2func(equ2);
        solver2 = RootSolver(f2, iter, pre);
        [numOfIterations,executionTime,allIteration,aproxRoot,precision,theoError,message] = solver2.FixedPoint(lower);
        set(handles.out_msg, 'String', message);
        if(strcmp(message, 'Success'))
            rownames = {'TE','P','R','N','Ex'};
            a = [theoError;precision;aproxRoot;numOfIterations;executionTime];
...
set(handles.input_equation,'String',firstline);
...
% hObject    handle to out_msg (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)

% Hints: get(hObject,'String') returns contents of out_msg as text
%        str2double(get(hObject,'String')) returns contents of out_msg as a double
